 ...
 * 智谱 GLM provider 客户端。
 *
 * <p>GLM 文本/编码模型固定走 coding endpoint；GLM-5V 多模态模型固定走通用多模态 endpoint。</p>
 */
public class GlmClient extends OpenAiCompatibleChatClient {
    public static final String CODING_BASE_URL = "https://open.bigmodel.cn/api/coding/paas/v4";
    public static final String MULTIMODAL_BASE_URL = "https://open.bigmodel.cn/api/paas/v4";
    ...
    GlmClient(String apiKey, String model, String apiUrlOverride, OkHttpClient httpClient) {
        super(
            apiKey,
            model == null || model.isBlank() ? DEFAULT_MODEL : model,
            selectApiUrl(model == null || model.isBlank() ? DEFAULT_MODEL : model, apiUrlOverride),
            httpClient
        );
    }
    ...
    private static String selectApiUrl(String model, String apiUrlOverride) {
        if (apiUrlOverride != null && !apiUrlOverride.isBlank()) {
            return toChatCompletionsUrl(apiUrlOverride);
        }

# Bruce Coding Agent MCP 设计说明

本文介绍 Bruce Coding Agent 中 MCP（Model Context Protocol）能力的设计与每个类的职责。MCP 子系统位于 `src/main/java/com/brucecli/mcp`，目标是把第三方 MCP server 暴露的工具接入现有统一 CLI，而不是新建独立 demo 入口。

## 总体流程

MCP 集成链路如下：

1. `McpConfigLoader` 从 `~/.bruce/setting.json` 读取 MCP 配置。
...
MCP 配置加载器。MCP server 统一写在 `~/.bruce/setting.json` 的 `mcp.servers` 中：
...
- `${HOME}`：当前用户 home 目录。
- `${variables.name}`：读取 `setting.json` 的 `variables.name`。
- `${llm.providers.glm.apiKey}`：读取 `setting.json` 中的点路径字段。

`detectType()` 会把 `type=http`、`type=streamable_http`、`type=streamable-http`、`type=streamableHttp` 或存在 `url` 的配置识别为 HTTP，其余默认 stdio。
...
MCP 工具进入 Bruce Coding Agent 后的运行时描述。
...
- `setting.json` 中的 `mcp.servers` 会加载。
- `${PROJECT_DIR}`、`${HOME}`、`${variables.xxx}` 和 settings 点路径可替换。
- 旧的 `.bruce/mcp.json` 不再参与加载。
...
MCP 工具来自第三方 server，能力边界由 server 决定。它可能读写本地文件、访问远程服务或执行副作用操作。默认审批更符合 Bruce Coding Agent 已有的安全模型。
